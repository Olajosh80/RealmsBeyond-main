...
export function ProductEditor() {
    ...
    return (
        <div className="flex flex-col gap-6 relative">
            {/* Toolbar */}
            <div className="sticky top-0 z-10 bg-white/80 backdrop-blur-xl border-b border-rare-primary/10 -mx-6 px-6 py-4 flex justify-between items-center shadow-sm">
            ...
            <div className="flex flex-col lg:flex-row gap-6 mt-4">
                ...
                <div className="flex-1 space-y-6">
                    ...
                    <div className="space-y-3">
                        <input
                            ...
                            className="w-full text-3xl font-heading font-bold p-4 bg-white/60 backdrop-blur-md border border-white/40 rounded-xl focus:outline-none focus:ring-2 focus:ring-rare-primary/50 placeholder-gray-400 shadow-sm"
                        />
                        <div className="flex items-center gap-2 text-sm text-gray-500 px-2">
                            ...
                            <input
                                ...
                                value={product.slug}
                                onChange={handleInputChange}
                                className="bg-transparent border-b border-dashed border-gray-300 hover:border-rare-primary focus:border-rare-primary focus:outline-none text-gray-700 font-mono text-xs py-0.5 min-w-[200px]"
                            />
                        </div>
                    </div>

                    {/* Description - Simplified wysiwyg placeholder */}
                    <div className="bg-white/60 backdrop-blur-md border border-white/40 rounded-xl overflow-hidden shadow-sm min-h-[300px] flex flex-col">
                        <div className="bg-gray-50/50 border-b border-gray-100 p-2 flex gap-2">
                            <button className="p-1.5 hover:bg-white rounded text-gray-600 font-bold text-xs border border-transparent hover:border-gray-200">Bold</button>
                            <button className="p-1.5 hover:bg-white rounded text-gray-600 italic text-xs border border-transparent hover:border-gray-200">Italic</button>
                        </div>
                        <textarea
                            name="description"
                            ...
                            onChange={handleInputChange}
                            className="flex-1 w-full p-4 bg-transparent border-none focus:ring-0 resize-none outline-none font-body text-gray-700 leading-relaxed"
                        />
                    </div>

                    {/* Product Data Meta Box */}
                    <div className="bg-white/80 backdrop-blur-md border border-white/40 rounded-xl shadow-lg run-in">
                        <div className="border-b border-gray-100 p-4 bg-gray-50/80 flex items-center justify-between">
                            <h3 className="font-bold text-gray-800 flex items-center gap-2">Product Data</h3>
                        </div>

                        <div className="flex flex-col md:flex-row min-h-[250px]">
                            {/* Tabs */}
                            <div className="w-full md:w-48 bg-gray-50/50 border-r border-gray-100 p-2 space-y-1">
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-2.5 text-sm font-medium flex items-center gap-3 rounded-lg transition-all ${activeTab === tab.id ? 'bg-white text-rare-primary shadow-sm ring-1 ring-black/5' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'}`}
                                    >
                                    ...
                                    <div className="space-y-4 max-w-sm">
                                        ...
                                        <div className="flex items-center gap-2 pt-2">
                                            <input
                                                ...
                                                onChange={handleInputChange}
                                                className="rounded border-gray-300 text-rare-primary focus:ring-rare-primary"
                                            />
                ...
                <div className="w-full lg:w-80 space-y-6">
                    ...
                    {/* Product Image */}
                    <div className="bg-white/80 backdrop-blur-md border border-white/40 rounded-xl shadow-lg p-5">
                        <h3 className="font-bold text-gray-800 border-b border-gray-100 pb-3 mb-3 text-sm uppercase tracking-wide">Featured Image</h3>
                        <input
                            type="file"
                        ...
                        {product.images.length > 0 ? (
                            ...
                            <button
                                onClick={() => featuredImageInputRef.current?.click()}
                                className="w-full aspect-square bg-gray-50 border-2 border-dashed border-gray-200 rounded-lg flex flex-col items-center justify-center text-gray-400 hover:text-rare-primary hover:border-rare-primary hover:bg-rare-primary/5 transition-all gap-2"
                            >
                                <FiImage size={32} />
                    ...
                    {/* Categories */}
                    <div className="bg-white/80 backdrop-blur-md border border-white/40 rounded-xl shadow-lg p-5">
                        <h3 className="font-bold text-gray-800 border-b border-gray-100 pb-3 mb-3 text-sm uppercase tracking-wide">Categories</h3>
                        <div className="max-h-60 overflow-y-auto space-y-1 my-3 pr-2 scrollbar-thin scrollbar-thumb-gray-200">
                            {categories.map(cat => (
                                <label key={cat._id} className="flex items-center gap-2.5 text-sm text-gray-700 cursor-pointer hover:bg-gray-50 p-1.5 rounded transition-colors select-none">
                                    <input
                                        type="radio" // Assuming single category for now based on model, or change to checkbox
                                        name="category"
                                        checked={product.category === cat.name}
                                        onChange={() => setProduct(p => ({ ...p, category: cat.name }))}
                                        className="text-rare-primary focus:ring-rare-primary border-gray-300 w-4 h-4"
                                    />
                                    {cat.name}
                                </label>
                            ))}
                        ...
                        {isCategoryModalOpen && (
                            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm animate-in fade-in duration-200">
                                <div className="bg-white/90 backdrop-blur-xl border border-white/50 p-6 rounded-2xl shadow-2xl max-w-sm w-full space-y-4 animate-in zoom-in-95 duration-200">
                                    <h3 className="text-lg font-heading font-bold text-gray-800">New Category</h3>
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="w-full px-4 py-2 bg-white/50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rare-primary/50 transition-all font-medium text-gray-800"
                                            autoFocus
                                            onKeyDown={(e) => {
                                    ...
                                    <div className="flex justify-end gap-2 pt-2">
                                        <button
                                            ...
                                            className="px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-700 hover:bg-gray-100/50 rounded-lg transition-colors"
                                        >
                                            Cancel
                                        </button>
                                        <button
                    ...
                    {/* Tags */}
                    <div className="bg-white/80 backdrop-blur-md border border-white/40 rounded-xl shadow-lg p-5">
                        <h3 className="font-bold text-gray-800 border-b border-gray-100 pb-3 mb-3 text-sm uppercase tracking-wide">Tags</h3>
                        <div className="flex gap-2">
                            <input
                                ...
                                className="flex-1 text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-rare-primary"
                            />
                            <button
                                onClick={handleAddTag}
                                className="px-3 py-2 text-sm bg-gray-100 border border-gray-200 rounded-lg hover:bg-gray-200 font-medium transition-colors"
                            >
                                Add
                            </button>
                        </div>
                        {product.tags.length > 0 && (
                            <div className="flex flex-wrap gap-2 mt-4">
                                {product.tags.map(tag => (
                                    <span key={tag} className="bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded-md flex items-center gap-1 border border-gray-200">
                    ...
                    {/* Product Gallery */}
                    <div className="bg-white/80 backdrop-blur-md border border-white/40 rounded-xl shadow-lg p-5">
                        <h3 className="font-bold text-gray-800 border-b border-gray-100 pb-3 mb-3 text-sm uppercase tracking-wide">Gallery</h3>
                        <input
                            type="file"
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-3">
                            {product.images.slice(1).map((img, i) => (
                                <div key={i} className="aspect-square rounded border overflow-hidden relative group">
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white/90 backdrop-blur-xl border border-white/50 p-6 rounded-2xl shadow-2xl max-w-sm w-full space-y-4 animate-in zoom-in-95 duration-200">
                        ...
                        <div className="text-center space-y-2">
                            <h3 className="text-lg font-heading font-bold text-gray-800">{messageState.title}</h3>
